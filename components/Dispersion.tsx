...
export default function Dispersion() {
  ...
  return (
    <div className="legend-frame rv in">
      ...
      {/* Desktop: SVG prism schematic */}
      <div
        className={`legend-svg-wrap ${shown ? "legend dimming" : "legend"}`}
        style={{ padding: 0 }}
      >
        <svg
          viewBox="0 0 980 200"
        ...
        >
          ...
          {wavelengths.map((w, i) => (
            ...
            >
              <line x1={380} y1={START_Y[i]} x2={LINE_END_X} y2={END_Y[i]}
                stroke={w.cssVar} strokeWidth={1.5} />
      ...
      {/* Mobile: 2×2 tappable band grid */}
      <div className="legend-band-grid">
        {wavelengths.map((w) => (
          <button
            key={w.nm}
            className={`band-tile${shown?.nm === w.nm ? " on" : ""}`}
            style={{ "--band-c": w.cssVar } as React.CSSProperties}
            onClick={() => toggleLock(w)}
            aria-pressed={locked?.nm === w.nm}
          >
            <span className="tile-nm">{w.nm}</span>
            <span className="tile-name">{w.name}</span>
            {w.aug && <span className="tile-aug">AUG</span>}
          </button>
        ))}
      </div>
      ...
      <p className="legend-read">
        {shown ? (
          <>
            <b style={{ color: shown.cssVar }}>{shown.nm} — {shown.name}.</b>{" "}
            {shown.aug && (
              <span className="aug-chip" style={{ color: shown.cssVar }}>AUG</span>
            )}{" "}
            {shown.copy}
          </>
        ) : (
